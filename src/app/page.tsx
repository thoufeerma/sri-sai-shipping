  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 md:top-8 md:left-8 z-20 w-10 h-10 md:w-14 md:h-14 bg-[var(--color-bg-main)]/70 backdrop-blur-md rounded-xl md:rounded-2xl flex items-center justify-center border border-white/10 group-hover:border-[var(--color-accent-cyan)]/30 group-hover:bg-[var(--color-primary)]/50 transition-all duration-500">
        <Icon className="text-white group-hover:text-[var(--color-accent-cyan)] group-hover:scale-110 transition-all duration-500 w-5 h-5 md:w-6 md:h-6" strokeWidth={1.5} />
      </div>

      {/* Bottom Text Content */}
      <div className="absolute bottom-0 left-0 w-full p-4 md:p-8 z-20 flex flex-col justify-end">
        <h3 className="text-sm md:text-2xl font-bold text-white mb-1 md:mb-2 leading-tight group-hover:text-[var(--color-accent-cyan)] transition-colors duration-300">
          {title}
        </h3>
        
        <div className="h-0 opacity-0 group-hover:h-auto group-hover:opacity-100 transition-all duration-500 overflow-hidden">
          <p className="theme-text text-[10px] md:text-sm mt-1 md:mt-3 leading-relaxed line-clamp-3 md:line-clamp-none">
            {desc}
          </p>
        </div>
      </div>
...
export default function Home() {
  ...
  return (
    <div ref={containerRef} className="flex flex-col w-full">
      ...
      {/* ─── STATS STRIP ─── */}
      <section className="py-16 md:py-20 border-b border-white/10 bg-gradient-to-br from-[#061A24] via-[#0B2430] to-[#071821] relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[var(--color-primary)]/10 via-transparent to-transparent pointer-events-none" />
        <AmbientGradient />
        <div className="max-w-7xl mx-auto px-5 sm:px-8 md:px-12 relative z-10">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 text-center">
            {[
              ...
              <div key={i} className="stat-num theme-card p-6 rounded-2xl">
                <div className="text-3xl md:text-5xl font-light theme-heading mb-1 md:mb-2">{stat.num}</div>
                <div className="text-xs md:text-sm theme-text tracking-wide">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="svc-section py-16 md:py-24 border-t border-white/10 bg-[var(--color-bg-main)] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 md:px-12 relative z-10">
          <div className="mb-10 md:mb-16">
            <p className="theme-text text-xs tracking-[0.4em] uppercase mb-3 md:mb-4">CORE CAPABILITIES</p>
            <AnimatedText as="h2" className="text-3xl md:text-5xl font-medium theme-heading tracking-tight max-w-2xl">
          ...
          <div className="mt-8 md:mt-10 text-center">
            <MagneticButton>
              <Link href="/services" className="inline-flex items-center gap-2 theme-text hover:text-[var(--color-accent-cyan)] transition-colors text-sm font-semibold tracking-wide border-b border-transparent hover:border-[var(--color-accent-cyan)] pb-0.5">
      ...
      <section className="py-12 md:py-24 border-t border-white/10 bg-gradient-to-br from-[#061A24] via-[#0B2430] to-[#071821] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 md:px-12 relative z-10">
          <div className="mb-12 md:mb-16">
            <p className="theme-text text-xs tracking-[0.4em] uppercase mb-3 md:mb-4">INDUSTRY EXPERTISE ACROSS GLOBAL SUPPLY CHAINS</p>
            <AnimatedText as="h2" className="text-3xl md:text-5xl font-medium theme-heading tracking-tight max-w-3xl">
              Specialized Logistics for Diverse Sectors
            </AnimatedText>
            <p className="theme-text text-sm mt-4 max-w-2xl">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {industries.map((ind, i) => (
              <div 
                key={i} 
                className={`w-[calc(50%-12px)] sm:w-[calc(50%-12px)] flex ${
      ...
      <section className="py-12 md:py-24 relative overflow-hidden border-t border-white/10 theme-section shadow-inner">
        <AmbientGradient />
        <div className="max-w-7xl mx-auto px-5 sm:px-8 md:px-12 relative z-10">
          <div className="text-center mb-10 md:mb-16">
            <p className="theme-text text-xs tracking-[0.4em] uppercase mb-3 md:mb-4">INTERNATIONAL TRUSTED ORGANIZATIONS</p>
            <AnimatedText as="h2" className="text-2xl md:text-4xl font-medium theme-heading tracking-tight mx-auto max-w-2xl">
              Trusted Clients & Organizations
            </AnimatedText>
            <p className="theme-text text-sm mt-4 max-w-xl mx-auto">
            ...
            {[
              ...
              <div key={i} className={`group relative rounded-2xl md:rounded-3xl overflow-hidden shadow-lg hover:shadow-[0_20px_40px_rgba(0,113,156,0.15)] hover:-translate-y-1.5 transition-all duration-500 min-h-[240px] md:min-h-[380px] flex flex-col justify-end theme-card hover:theme-card-hover ${i === 2 ? "col-span-2 md:col-span-1" : ""}`}>
                ...
                <div className="relative z-20 p-4 md:p-8 w-full">
                  ...
                  <div className="h-0 opacity-0 group-hover:h-auto group-hover:opacity-100 transition-all duration-500 overflow-hidden">
                    <p className="theme-text text-[10px] md:text-sm leading-relaxed drop-shadow-sm border-t border-white/10 pt-2 md:pt-4 mt-1 md:mt-2 line-clamp-2 md:line-clamp-none">{client.desc}</p>
                  </div>
                </div>
              </div>
            ))}
