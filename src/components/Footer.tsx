...
export default function Footer() {
  return (
    <footer className="border-t border-white/10 bg-gradient-to-b from-[#0B2430] to-[#061A24] pt-20 pb-10">
      <div className="max-w-7xl mx-auto px-6">

        {/* Top strip with image accent */}
        <div className="relative h-32 rounded-2xl overflow-hidden mb-16 border border-white/10">
          <Image src="/hero-port.webp" alt="Sri Sai port" fill className="object-cover object-bottom opacity-20" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          <div className="lg:col-span-1">
            <p className="text-sm theme-text leading-relaxed mb-4">
          ...
          <div>
            <h4 className="text-white/80 text-xs font-semibold tracking-widest uppercase mb-4">Navigation</h4>
            <ul className="space-y-3">
              {["Home", "About", "Services", "Global Logistics", "Contact"].map((link) => (
                <li key={link}>
                  <Link
                    href={link === "Home" ? "/" : `/${link.toLowerCase().replace(" ", "-")}`}
                    className="theme-text hover:text-[var(--color-accent-cyan)] transition-colors text-sm flex items-center gap-1 group w-fit"
                  >
                    {link}
          ...
          <div>
            <h4 className="text-white/80 text-xs font-semibold tracking-widest uppercase mb-4">Capabilities</h4>
            <ul className="space-y-3">
              {["Customs Broking & Logistics Services", "Freight Forwarding", "Air Freight", "Transportation", "Smart Warehousing", "Coastal Services", "Vessel Chartering", "Equipment Leasing"].map((s) => (
                <li key={s} className="theme-text text-sm">{s}</li>
              ))}
            </ul>
          </div>

          <div>
            <h4 className="text-white/80 text-xs font-semibold tracking-widest uppercase mb-4">Contact</h4>
            <p className="theme-text text-sm leading-relaxed mb-3">
              <span className="font-semibold text-white">SRI SAI SHIPPING AGENCIES</span><br />
              D. No. 14/1284 D<br />
